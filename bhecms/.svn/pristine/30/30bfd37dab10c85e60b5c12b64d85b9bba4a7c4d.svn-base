...
SWFUpload.prototype.getFlashVars = function() {
	...
	return [ "movieName=", encodeURIComponent(this.movieName), "&amp;uploadURL=", encodeURIComponent(this.settings.upload_url), "&amp;useQueryString=", encodeURIComponent(this.settings.use_query_string), "&amp;requeueOnError=", encodeURIComponent(this.settings.requeue_on_error), "&amp;httpSuccess=", encodeURIComponent(a), "&amp;assumeSuccessTimeout=", encodeURIComponent(this.settings.assume_success_timeout), "&amp;params=", encodeURIComponent(b), "&amp;filePostName=", encodeURIComponent(this.settings.file_post_name), "&amp;fileTypes=", encodeURIComponent(this.settings.file_types), "&amp;fileTypesDescription=", encodeURIComponent(this.settings.file_types_description), "&amp;fileSizeLimit=", encodeURIComponent(this.settings.file_size_limit), "&amp;fileUploadLimit=",
			encodeURIComponent(this.settings.file_upload_limit), "&amp;fileQueueLimit=", encodeURIComponent(this.settings.file_queue_limit), "&amp;debugEnabled=", encodeURIComponent(this.settings.debug_enabled), "&amp;buttonImageURL=", encodeURIComponent(this.settings.button_image_url), "&amp;buttonWidth=", encodeURIComponent(this.settings.button_width), "&amp;buttonHeight=", encodeURIComponent(this.settings.button_height), "&amp;buttonText=", encodeURIComponent(this.settings.button_text), "&amp;buttonTextTopPadding=", encodeURIComponent(this.settings.button_text_top_padding), "&amp;buttonTextLeftPadding=", encodeURIComponent(this.settings.button_text_left_padding), "&amp;buttonTextStyle=", encodeURIComponent(this.settings.button_text_style), "&amp;buttonAction=",
			encodeURIComponent(this.settings.button_action), "&amp;buttonDisabled=", encodeURIComponent(this.settings.button_disabled), "&amp;buttonCursor=", encodeURIComponent(this.settings.button_cursor) ].join("")
};
		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					swf : 'uploadify.swf', // The path to the uploadify SWF
											// file
					uploader : 'uploadify.php', // The path to the server-side
												// upload script

					// Options
					auto : true, // Automatically upload files when added to
									// the queue
					buttonClass : '', // A class name to add to the browse
										// button DOM object
					buttonCursor : 'hand', // The cursor to use with the browse
											// button
					...
					fileObjName : 'Filedata', // The name of the file object
												// to use in your server-side
												// script
					fileSizeLimit : 0, // The maximum size of an uploadable
										// file in KB (Accepts units B KB MB GB
										// if string, 0 for no limit)
					fileTypeDesc : 'All Files', // The description for file
												// types in the browse dialog
					fileTypeExts : '*.*', // Allowed extensions in the browse
											// dialog (server-side validation
											// should also be used)
					height : 30, // The height of the browse button
					itemTemplate : false, // The template for the file item in
											// the queue
					method : 'post', // The method to use when sending files
										// to the server-side upload script
					multi : true, // Allow multiple file selection in the
									// browse dialog
					formData : {}, // An object with additional data to send to
									// the server-side upload script with every
									// file upload
					preventCaching : true, // Adds a random value to the Flash
											// URL to prevent caching of it
											// (conflicts with existing
											// parameters)
					progressData : 'percentage', // ('percentage' or 'speed')
													// Data to show in the queue
													// item during a file upload
					queueID : false, // The ID of the DOM object to use as a
										// file queue (without the #)
					queueSizeLimit : 999, // The maximum number of files that
											// can be in the queue at one time
					removeCompleted : true, // Remove queue items from the queue
											// when they are done uploading
					removeTimeout : 3, // The delay in seconds before removing
										// a queue item if removeCompleted is
										// set to true
					requeueErrors : false, // Keep errored files in the queue
											// and keep trying to upload them
					successTimeout : 30, // The number of seconds to wait for
				 ...
				 * onFallback // Triggered is Flash is not detected onInit //
				 * Triggered when Uploadify is initialized onQueueComplete //
				 * Triggered when all files in the queue have been uploaded
				 * onSelectError // Triggered when an error occurs while
				 * selecting a file (file size, queue size limit, etc.) onSelect //
				 * Triggered for each file that is selected onSWFReady //
				 * Triggered when the SWF button is loaded onUploadComplete //
				 * Triggered when a file upload completes (success or error)
				 * onUploadError // Triggered when a file upload returns an
				 * error onUploadSuccess // Triggered when a file is uploaded
				 * successfully onUploadProgress // Triggered every time a file
				 * progress is updated onUploadStart // Triggered immediately
				 * before a file upload starts
				 */
				}, options);
				...
				if (flashInstalled) {
					...
					swfuploadify.queueData = {
						...
						filesSelected : 0, // The number of files selected in
											// the last select operation
						filesQueued : 0, // The number of files added to the
											// queue in the last select
											// operation
						filesReplaced : 0, // The number of files replaced in
											// the last select operation
						filesCancelled : 0, // The number of files that were
											// cancelled instead of replaced
						filesErrored : 0, // The number of files that caused
											// error in the last select
											// operation
						uploadsSuccessful : 0, // The number of files that were
												// successfully uploaded
						uploadsErrored : 0, // The number of files that returned
											// errors during upload
						averageSpeed : 0, // The average speed of the uploads
											// in KB
